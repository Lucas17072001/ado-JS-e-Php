...
function nomesDosAlunos() {
    return ["Lucas Noe", "Matheus Ferrari", "Luan da Silva", "Gabriel Grau"];
}
 ...
 * Fevereiro tem 28 dias em anos não bissextos e 29 em bissextos.
...
function dataValida(data) {
    let dataParte = data.replaceAll(" ", "X").split("/");

    if (dataParte.length !== 3) return false;
    if (dataParte[0].length !== 2 || dataParte[1].length !== 2 || dataParte[2].length !== 4) return false;
    const dia = Number(dataParte[0]);
    const mes = Number(dataParte[1]);
    const ano = Number(dataParte[2]);
    if (isNaN(dia) || isNaN(mes) || isNaN(ano)) return false;

    if (dia <= 0 || dia > 31 || mes <= 0 || mes > 12 || ano <= 0 || ano > 9999) return false;
    if (dia == 31 && (mes == 2 || mes == 4 || mes == 6 || mes == 9 || mes == 11)) return false;
    if (dia == 30 && mes == 2) return false;
    if (dia <= 28 || mes !== 2) return true;
    const excecao = ano % 100 === 0 && ano % 400 !== 0;
    if (ano % 4 !== 0 || excecao) return false;
    return true;
}
...
function converteDataParaFormaCompleta(data) {
    ...
    if (dataParte.length !== 3) return "Data inválida";
    if (dataParte[0].length !== 2 || dataParte[1].length !== 2 || dataParte[2].length !== 4) return "Data inválida";
    const dia = Number(dataParte[0]);
    const mes = Number(dataParte[1]);
    const ano = Number(dataParte[2]);
    if (isNaN(dia) || isNaN(mes) || isNaN(ano)) return "Data inválida";
    if (dia <= 0 || dia > 31 || mes <= 0 || mes > 12 || ano <= 0 || ano > 9999) return "Data inválida";
    if (dia == 31 && (mes == 2 || mes == 4 || mes == 6 || mes == 9 || mes == 11)) return "Data inválida";
    const excecao = ano % 100 === 0 && ano % 400 !== 0;
    if ((dia > 28 && mes == 2) && (ano % 4 !== 0 || excecao)) return "Data inválida";
    if (dia > 29 && mes == 2) return "Data inválida";


    if (mes == 1) return ("00" + dia).slice(-2) + " de Janeiro de " + ("0000" + ano).slice(-4);
    if (mes == 2) return ("00" + dia).slice(-2) + " de Fevereiro de " + ("0000" + ano).slice(-4);
    if (mes == 3) return ("00" + dia).slice(-2) + " de Março de " + ("0000" + ano).slice(-4);
    if (mes == 4) return ("00" + dia).slice(-2) + " de Abril de " + ("0000" + ano).slice(-4);
    if (mes == 5) return ("00" + dia).slice(-2) + " de Maio de " + ("0000" + ano).slice(-4);
    if (mes == 6) return ("00" + dia).slice(-2) + " de Junho de " + ("0000" + ano).slice(-4);
    if (mes == 7) return ("00" + dia).slice(-2) + " de Julho de " + ("0000" + ano).slice(-4);
    if (mes == 8) return ("00" + dia).slice(-2) + " de Agosto de " + ("0000" + ano).slice(-4);
    if (mes == 9) return ("00" + dia).slice(-2) + " de Setembro de " + ("0000" + ano).slice(-4);
    if (mes == 10) return ("00" + dia).slice(-2) + " de Outubro de " + ("0000" + ano).slice(-4);
    if (mes == 11) return ("00" + dia).slice(-2) + " de Novembro de " + ("0000" + ano).slice(-4);
    if (mes == 12) return ("00" + dia).slice(-2) + " de Dezembro de " + ("0000" + ano).slice(-4);
}
...
function acharMenor(vetor) {
    let verificaMenor = vetor[0];

    if (verificaMenor !== undefined) {
        for (let index = 0; index < vetor.length; index++) {
            if (verificaMenor > vetor[index]) {
                verificaMenor = vetor[index];
            }
        }
...
function rot13(texto) {
    const letrasOriginais = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
    const letrasCodificadas = "NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm";

    let textoCodificado = "";

    for (let i = 0; i < texto.length; i++) {
        const letra = texto[i];
        // Verifica se a letra está presente no conjunto de letras originais
        const indice = letrasOriginais.indexOf(letra);
        if (indice !== -1) {
            // Se estiver, adiciona a letra correspondente do conjunto de letras codificadas
            textoCodificado += letrasCodificadas[indice];
        } else {
            // Se não estiver, adiciona a própria letra sem codificação
 ...
 *
 * @param {Object} times O dicionário contendo os nomes dos times e o respectivo número de pontos e saldo de gols.
